...
//------------------------------------------------------------------------
//int	parser::GetPacketID(sRmi *rmi, sProtocol *packet)
//{
//	if (!rmi) return 0;
 ...
 */
void parser::ReleaseRmiOnly(sRmi *p)
{
	if (!p) return;
...
/**
 @brief 
 */
void parser::ReleaseProtocol(sProtocol *p)
{
	if (!p) return;
...
/**
 @brief 
 */
void parser::ReleaseCurrentProtocol(sProtocol *p)
{
	if (!p) return;
	ReleaseArg(p->argList);
	delete p;
